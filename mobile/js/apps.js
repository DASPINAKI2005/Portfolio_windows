...
(function (global) {
    ...
    const PROJECTS = [
        {
            title: 'Nova — Local AI Chatbot',
            views: '4.2K',
            time: '2 days ago',
            dur: '3:24',
            c1: '#34d399',
            c2: '#0ea5e9',
            repo: 'https://github.com/DASPINAKI2005/groq-ai-chatbot',
            tagline: 'A local AI chatbot with streaming conversations, file analysis, web search and deep research.',
            desc: 'Nova is a fast, privacy-first chatbot that runs streaming conversations against the Groq API, analyses uploaded files, searches the web and composes deep-research answers - all through a clean FastAPI backend with a responsive web UI.',
            stack: ['Python', 'FastAPI', 'Groq API', 'LLM', 'JavaScript'],
            points: [
                'Streaming chat responses on ultra-low-latency Groq inference',
                'File upload and analysis for document-driven answers',
                'Web search and multi-turn research conversations',
                'Responsive UI backed by a clean, maintainable FastAPI service'
            ]
        },
        {
            ...
            c1: '#60a5fa',
            c2: '#a78bfa',
            repo: 'https://github.com/DASPINAKI2005/opencv',
            tagline: 'Real-time object detection in the browser powered by OpenCV computer vision.',
            desc: 'A real-time computer vision web app that detects and labels objects from a live camera feed. A Python + OpenCV pipeline renders detections with bounding boxes to the browser at streaming latency.',
            stack: ['Python', 'OpenCV', 'Flask', 'JavaScript', 'HTML/CSS'],
            points: [
                'Live camera feed with real-time object labelling',
                'OpenCV detection pipeline with bounding-box rendering',
                'Low-latency streaming from backend to browser',
                'Clean separation between the CV pipeline and the UI'
            ]
        },
        {
            title: 'The Eighth Wonder — AI Project Idea Generator',
            views: '2.1K',
            time: '3 weeks ago',
            dur: '6:12',
            c1: '#f472b6',
            c2: '#fb923c',
            repo: 'https://github.com/DASPINAKI2005/idea-gen',
            tagline: 'Recruiter-focused portfolio project ideas generated by a fully local LLM.',
            desc: 'The Eighth Wonder generates structured, recruiter-friendly project ideas from your skills, experience level and career goal. A carefully engineered prompt drives a local Llama 3.2 model through Ollama - no API keys, no cloud, nothing leaves the machine.',
            stack: ['Python', 'Flask', 'Ollama', 'Llama 3.2', 'JavaScript'],
            points: [
                'Fully local LLM inference - zero API cost, full privacy',
                'Structured ideas: problem, features, stack, difficulty and timeline',
                'Engineered prompt pipeline for consistent, useful output',
                'Premium responsive UI with markdown-rendered results'
            ]
        }
    ];
    ...
    const PHOTOS = [
        { name: 'Introduction to AI', kind: 'cert', grad: 'linear-gradient(135deg,#60a5fa,#818cf8)' },
        { name: 'Productivity with AI Tools', kind: 'cert', grad: 'linear-gradient(135deg,#34d399,#0ea5e9)' },
        { name: 'AWS Power Hour: GenAI', kind: 'cert', grad: 'linear-gradient(135deg,#f472b6,#fb923c)' },
        { name: 'PyTorch & DL (LFS116)', kind: 'cert', grad: 'linear-gradient(135deg,#a78bfa,#ec4899)' },
        { name: 'Planning a GenAI Project', kind: 'cert', grad: 'linear-gradient(135deg,#fbbf24,#f59e0b)' },
        { name: 'Nova — AI Chatbot', kind: 'project', grad: 'linear-gradient(135deg,#38bdf8,#10b981)' },
        { name: 'Object Detection App', kind: 'project', grad: 'linear-gradient(135deg,#2dd4bf,#6366f1)' },
        { name: 'Eighth Wonder Generator', kind: 'project', grad: 'linear-gradient(135deg,#fb7185,#f59e0b)' },
        { name: 'Profile Photo', kind: 'profile', grad: 'linear-gradient(135deg,#64748b,#0f172a)' }
    ];
    ...
    const ACTIONS = {
        ...
        'open-photo': function (target) {
            ...
            if (cert) {
                ...
                const art = p.kind === 'project'
                    ? '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v12"/><path d="M12 14l-5-3v4.2A6 6 0 0 0 12 21a6 6 0 0 0 5-2.8V11z"/><circle cx="12" cy="2" r="2.2" fill="currentColor" stroke="none"/></svg>'
                    : '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="4"/><path d="M4 20c1.6-3.4 4.6-5 8-5s6.4 1.6 8 5"/></svg>';
                const label = p.kind === 'project' ? 'Portfolio project' : (p.kind === 'profile' ? 'Profile' : 'Collection');
                stage.innerHTML = '<div class="insta__art"><span class="insta__art-glyph">' + art + '</span><strong>' + esc(p.name) + '</strong><span class="insta__art-label">' + label + '</span></div>';
            }
